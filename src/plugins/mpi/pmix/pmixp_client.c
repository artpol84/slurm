...
{
	int i;
	uint32_t *node_offs = xmalloc(nsptr->nnodes * sizeof(*node_offs));
	uint32_t *node_tasks = xmalloc(nsptr->nnodes * sizeof(*node_tasks));
	...
	xassert(nsptr->ntasks ==
		(node_offs[nsptr->nnodes-1] + nsptr->task_cnts[nsptr->nnodes-1]));

	/* Fill the node-to-task map */
	for(i = 0; i < nsptr->ntasks; i++) {
		int node = nsptr->task_map[i];
		xassert(node < nsptr->nnodes);
		int offset = node_offs[node] + node_tasks[node]++;
...
static int _set_mapsinfo(List lresp)
{
	...
	node2tasks = xmalloc(nsptr->ntasks * sizeof(*node2tasks));
	_build_node2task_map(nsptr, node2tasks);
	cur_task = node2tasks;
	for (i = 0; i < nsptr->nnodes; i++) {
		char *sep = "";
		/* For each node, provide ID's of the tasks residing on it */
